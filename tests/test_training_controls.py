...
import mimic_comal.multimodal_training as training_module
...
def test_formal_config_enables_the_round_training_controls() -> None:
    config = load_config(Path(__file__).parents[1] / "configs/mimic_comal.yaml")
...
def test_full_cohort_config_remains_the_yang_wu_task() -> None:
    config = load_config(Path(__file__).parents[1] / "configs/mimic_full_cohort.yaml")
