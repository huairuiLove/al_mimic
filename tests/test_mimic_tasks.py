...
from mimic_comal.metrics import ranking_metrics, task_multilabel_metrics
from mimic_comal.model import YangWuBertEncoderClassifier
from mimic_comal.multimodal_data import audit_split_hdf5
from mimic_comal.tasks import TASKS, task_spec
from scripts.build_ccs_172_labels import build_ccs_labels
...
@pytest.mark.parametrize(
    ...
    [
        ("configs/mimic_comal.yaml", "icd9_diagnoses", 915, "recall_at_30"),
        ("configs/mimic_phenotyping_25.yaml", "phenotyping_25", 25, "macro_auprc"),
        (
            "configs/mimic_phenotyping_ccs_172.yaml",
            "phenotyping_ccs_172",
            172,
            "macro_auprc",
        ),
...
def test_phenotyping_metrics_are_ranking_metrics_not_top_k() -> None:
    ...
    probabilities = np.asarray(
        [[0.9, 0.1], [0.2, 0.8], [0.7, 0.6], [0.1, 0.2]], dtype=np.float32
    )
    direct = ranking_metrics(labels, probabilities)
    config = load_config(ROOT / "configs/mimic_phenotyping_25.yaml")
...
def test_two_modality_model_exposes_no_static_token() -> None:
    ...
    assert torch.allclose(
        model.fuse_from_tokens(output["modality_tokens"]), output["features"], atol=1e-6
    )
    ...
    with h5py.File(path, "w") as handle:
        ...
        for split_index, split in enumerate(("train", "val", "test")):
            ...
            group.create_dataset(
                "subject_id", data=np.asarray([10 * split_index + 1, 10 * split_index + 2])
            )
