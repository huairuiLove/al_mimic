...
def modality_thresholds(probabilities: torch.Tensor, prevalence: torch.Tensor) -> torch.Tensor:
    ...
    return torch.stack(
        [quantile_thresholds(probabilities[:, modality], prevalence) for modality in range(probabilities.shape[1])]
    )
...
def modality_sufficiency(
    ...
    if modality_positive.shape[0] != fused_positive.shape[0] or modality_positive.shape[2] != fused_positive.shape[1]:
    ...
    for modality in range(tokens.shape[1]):
        ...
        if bisect_steps and ever_changed.any():
            ...
            for _ in range(bisect_steps):
                ...
                intervened = interpolate_modality_token(
                    active_tokens, prototypes, modality, midpoint
                )
                fused = fuse_token_batches(classifier, intervened, batch_size=fusion_batch_size)
                decisions = probabilities_from_fused(classifier, fused) >= thresholds
...
def _component_quantiles(values: torch.Tensor) -> dict[str, float]:
    ...
    return {
        name: float(value)
        for name, value in zip(("min", "q25", "median", "q75", "q90", "max"), quantiles)
    }
...
def acquire_modis(
    ...
    *,
    ...
    probe_probs = probe_probabilities(
        probe_state.probes, tokens, batch_size=probe_batch_size
    )
    ...
    screen_score = _average_rank_fraction(disagreement) * _average_rank_fraction(
        sufficiency_penalty
    )
    requested_workset = int(modis_cfg.get("workset_size", 5000))
    workset_count = min(
        probabilities.shape[0], max(int(query_size), max(1, requested_workset))
    )
    ...
    combined = _geometric_rank_score(
        (disagreement, instability, sufficiency_penalty), beta
    )
    ...
    ranked_work = torch.argsort(
        combined.index_select(0, work_positions), descending=True, stable=True
    )
    ...
    boundary_fraction = (
        float((selected_screen_ranks >= tail_start).float().mean())
        if selected_screen_ranks.numel()
        else 0.0
    )
    ...
    pairwise_correlations = {
        "disagreement_vs_instability": _spearman(
            work_disagreement, work_instability.combined
        ),
        "disagreement_vs_sufficiency_penalty": _spearman(
            work_disagreement, work_sufficiency_penalty
        ),
        "instability_vs_sufficiency_penalty": _spearman(
            work_instability.combined, work_sufficiency_penalty
        ),
    }
    ...
    if candidate_metadata:
        for name, values in candidate_metadata.items():
            metadata = torch.as_tensor(
                np.asarray(values, dtype=np.float32), device=dominance.device
            )
    ...
    diagnostics = {
        "criteria": {
            ...
            "instability_quantiles_on_workset": _component_quantiles(
                work_instability.combined
            ),
            "instability_zero_fraction_on_workset": float(
                (work_instability.combined == 0).float().mean()
            ),
            "instability_zero_fraction_by_modality": [
                float(value)
                for value in (work_instability.per_modality == 0).float().mean(dim=0)
            ],
        ...
        "intervention": {
            ...
            "monotonicity_violation_rate_by_modality": (
                work_instability.monotonicity_violation_rate
            ),
        ...
        "thresholds": {
            ...
            "predicted_to_labeled_cardinality_ratio": float(
                fused_positive.sum(dim=1).float().mean()
                / max(probe_state.labeled_cardinality, 1e-8)
            ),
