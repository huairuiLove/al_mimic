...
class ModalityProbes(nn.Module):
    ...
    def __init__(self, num_modalities: int, token_dim: int, num_labels: int) -> None:
        ...
        self.probes = nn.ModuleList(
            nn.Linear(token_dim, num_labels) for _ in range(num_modalities)
        )
...
def estimate_reliability_weights(
    ...
    *,
    ...
    baseline_loss = -(
        values * prevalence.log() + (1.0 - values) * (1.0 - prevalence).log()
    )
    probe_loss = -(
        values[:, None, :] * probabilities.log()
        + (1.0 - values[:, None, :]) * (1.0 - probabilities).log()
    )
    improvements = baseline_loss[:, None, :] - probe_loss
    information_gain = improvements.mean(dim=0)
    entropy = -(
        prevalence * prevalence.log() + (1.0 - prevalence) * (1.0 - prevalence).log()
    ).clamp_min(epsilon)
    ...
    *,
    ...
    with torch.random.fork_rng(devices=cuda_devices):
        ...
        if folds:
            ...
            oof_probabilities = prevalence[None, None, :].expand(
                labels.shape[0], tokens.shape[1], -1
            ).clone()
