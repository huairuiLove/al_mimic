...
class FisherDesign:
    ...
    def build(
        ...
        *,
        ...
        for scale in (1.0, 3.0, 10.0, 30.0, 100.0, 300.0, 1000.0):
            ...
            raise RuntimeError(
                f"Fisher blocks are not positive definite; increase mosaic.damping (tried up to {used:g})"
            )
        inverse = torch.cholesky_inverse(cholesky)
        residual = reference_probabilities.float() - reference_labels.float()
        reference_gradient = torch.einsum("nc,nd->cd", residual, reference) / max(int(reference.shape[0]), 1)
        direction = torch.einsum("cde,ce->cd", inverse, reference_gradient)
        return cls(
            inverse, reference_gradient, direction, information.diagonal(dim1=-2, dim2=-1).sum(-1), used
        )
    ...
    def deflate(self, feature: torch.Tensor, probabilities: torch.Tensor) -> None:
        ...
        self.inverse -= coefficient[:, None, None] * torch.einsum("cd,ce->cde", projected, projected)
