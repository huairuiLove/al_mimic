...
def train_modality_probes(
    ...
    *,
    ...
    epochs = int(modimix_cfg.get("probe_epochs", training_cfg.get("epochs", 20)))
    batch_size = int(modimix_cfg.get("probe_batch_size", training_cfg.get("batch_size", 512)))
    prediction_batch_size = int(
        modimix_cfg.get("probe_eval_batch_size", training_cfg.get("eval_batch_size", 1024))
    )
    fit_options = {
        "epochs": epochs,
        "batch_size": batch_size,
        "learning_rate": float(
            modimix_cfg.get("probe_learning_rate", training_cfg.get("learning_rate", 1e-3))
        ),
        "weight_decay": float(modimix_cfg.get("probe_weight_decay", training_cfg.get("weight_decay", 1e-4))),
    ...
    folds = _group_folds(group_array, int(modimix_cfg.get("oof_folds", 5)))
    cuda_devices = []
    ...
    prototypes, prototype_diagnostics = modality_prototypes(
        detached_tokens,
        method=str(modimix_cfg.get("prototype", "mean")),
        pairwise_sample_size=int(modimix_cfg.get("prototype_diagnostic_sample_size", 512)),
    )
    diagnostics = {
        "oof_folds": len(folds),
        ...
        "prototype": str(modimix_cfg.get("prototype", "mean")).lower(),
        "prototype_diagnostics": asdict(prototype_diagnostics),
    }
    return ModiMixProbeState(
        probes=full_probes,
