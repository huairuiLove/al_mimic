"""ModiMix disagreement, instability, and sufficiency-penalized acquisition."""
...
from .probes import ModiMixProbeState, probe_probabilities
...
@dataclass(frozen=True)
class ModiMixAcquisitionResult:
...
def acquire_modimix(
    ...
    *,
    ...
    if not callable(getattr(classifier, "fuse_from_tokens", None)):
        raise ValueError("ModiMix requires a classifier with fuse_from_tokens")
    ...
    modimix_cfg = config.get("modimix", {})
    beta_values = tuple(float(value) for value in modimix_cfg.get("beta", [1.0, 1.0, 1.0]))
    if len(beta_values) != 3:
        raise ValueError("modimix.beta must contain three values")
    beta = (beta_values[0], beta_values[1], beta_values[2])
    fusion_batch_size = int(modimix_cfg.get("fusion_batch_size", 4096))
    probe_batch_size = int(modimix_cfg.get("probe_eval_batch_size", fusion_batch_size))
    ...
    requested_workset = int(modimix_cfg.get("workset_size", 5000))
    ...
    work_instability = critical_instability(
        ...
        grid_k=int(modimix_cfg.get("grid_k", 8)),
        bisect_steps=int(modimix_cfg.get("bisect_steps", 0)),
    ...
    return ModiMixAcquisitionResult(
