"""Task-independent ModiMix plugin."""
...
from .acquire import acquire_modimix
from .probes import ModiMixProbeState, train_modality_probes
...
def _candidate_outputs(context: Any, fields: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(outputs, Mapping):
        raise ValueError("ModiMix candidate_outputs must be a tensor mapping")
    return dict(outputs)
...
class ModiMixPlugin:
    ...
    method_id: str = "modimix"
    display_name: str = "ModiMix"
    ...
    def fit(self, context: Any = None, **fields: Any) -> ModiMixProbeState:
        ...
        if groups is None:
            raise ValueError("ModiMix probe fitting requires row-aligned groups")
...
PLUGIN = ModiMixPlugin()


__all__ = ["ModiMixPlugin", "PLUGIN"]
