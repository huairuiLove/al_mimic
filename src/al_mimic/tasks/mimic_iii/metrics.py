...
def top_k_recall(labels: np.ndarray, probabilities: np.ndarray, k_values: Iterable[int]) -> dict[str, float]:
...
def ranking_metrics(labels: np.ndarray, probabilities: np.ndarray) -> dict[str, Any]:
    ...
    return {
        "macro_auprc": float(
            average_precision_score(labels[:, auprc_mask], probabilities[:, auprc_mask], average="macro")
        ),
