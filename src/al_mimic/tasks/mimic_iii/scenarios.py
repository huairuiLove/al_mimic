...
class ScenarioSpec:
    ...
    def summary(self) -> dict[str, Any]:
        ...
        if self.notes_missing is not None and self.series_missing is not None:
            payload["both_modalities_missing_rows"] = int((self.notes_missing & self.series_missing).sum())
...
def _label_columns(
    ...
    if drop_top_k > 0:
        if drop_top_k >= raw_count:
            raise ValueError(
                f"scenario.label_subset.drop_top_k={drop_top_k} would drop every one of {raw_count} labels"
            )
...
def _missing_weights(
    ...
    if bias == "careunit":
        if careunit_codes is None:
            raise ValueError(
                f"scenario.missing_{modality}.bias='careunit' needs a careunit_code array in the "
                "split artifact; rebuild the splits or choose another bias"
            )
        if careunit_target is None:
            raise ValueError(f"scenario.missing_{modality}.careunit must name the affected unit")
        weights = np.ones(eligible.size, dtype=np.float64)
        weights[careunit_codes[eligible] == careunit_target] = max(strength, 0.0) + 1.0
        return weights
    raise ValueError(f"scenario.missing_{modality}.bias must be one of {BIAS_MODES}, got {bias!r}")


def _modality_missing(
    modality: str,
    ...
    missing = np.zeros(total, dtype=bool) if natural_missing is None else natural_missing.copy()
    natural_rows = int(missing.sum())

    rate = float(settings.get("rate", 0.0))
    if not 0.0 <= rate < 1.0:
        raise ValueError(f"scenario.missing_{modality}.rate must lie in [0, 1), got {rate}")
    if rate <= 0.0:
        if natural_rows == 0:
            return None, {}
        return missing, {
    ...
    for split in splits:
        ...
        if not rows.any():
            raise ValueError(f"scenario.missing_{modality}.splits names an empty split: {split!r}")
        ...
        # Seed each split independently so resizing one split cannot shift another.
        split_seed = sum((index + 1) * ord(char) for index, char in enumerate(split))
...
def scenario_from_config(
    ...
    *,
    ...
    series_available: np.ndarray | None = None,
) -> ScenarioSpec:
    ...
    natural_notes_missing = None if notes_available is None else ~np.asarray(notes_available, dtype=bool)
    notes_missing, notes_provenance = _modality_missing(
        "notes",
        settings.get("missing_notes", {}) or {},
        labels,
        split_names,
        careunit_codes,
        natural_notes_missing,
    )
    natural_series_missing = None if series_available is None else ~np.asarray(series_available, dtype=bool)
    series_missing, series_provenance = _modality_missing(
        "series",
        settings.get("missing_series", {}) or {},
        labels,
        split_names,
        careunit_codes,
        natural_series_missing,
    )
    return ScenarioSpec(
        name=name,
        ...
        provenance=label_provenance | notes_provenance | series_provenance,
    )
