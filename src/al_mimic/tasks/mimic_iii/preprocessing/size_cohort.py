...
try:
    ...
    raise RuntimeError(
        "run size_cohort as a package module: python -m al_mimic.tasks.mimic_iii.preprocessing.size_cohort"
    ) from exc
...
def main() -> None:
    ...
    icus = pd.read_csv(args.data_dir / "prep" / "icustays_MV.csv", parse_dates=["INTIME", "OUTTIME"])
    ...
    for duration in args.durations:
        ...
        keep = {g for g, _ in counts.most_common(1042)} if len(counts) > 1042 else set(counts)
