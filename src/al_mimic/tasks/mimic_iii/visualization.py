...
def explore_dataset(config: dict[str, Any], output_dir: str | Path | None = None) -> dict[str, Any]:
    ...
    prepared = resolve_path(config, dataset.get("prepared_dir", "../dataset/prepared/yang_wu_diagnoses_48h"))
    ...
    axis.set(
        xlabel="Evaluation batch size",
        ylabel="One FP32 similarity matrix (MiB)",
        yscale="log",
    )
    axis.grid(alpha=0.2)
    axis.legend()
    fig.tight_layout()
    files["method_scaling"] = "method_memory_scaling.png"
    ...
    split_positive = {split: labels[store.indices(split)].sum(axis=0) for split in ("train", "val", "test")}
    ...
    audit["feature_feasibility"] = {
        "batch_tokens": configured * len(label_names),
        "similarity_matrix_mib": float((configured * len(label_names)) ** 2 * 4 / 1024**2),
        "recommendation": ("keep method-side batches bounded; scale evaluation batches independently"),
    }
...
def visualize_experiment(experiment_dir: str | Path, output_dir: str | Path | None = None) -> dict[str, Any]:
    ...
    metric_names = [
        name for name, value in records[0]["test_metrics"].items() if isinstance(value, (int, float))
    ]
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 4.4))
    has_auxiliary = any(
        any(name != "classifier_loss" and values for name, values in record["training_history"].items())
        for record in records
    )
    for round_index, record in enumerate(records):
        axes[0].plot(
            record["training_history"]["classifier_loss"],
            alpha=0.75,
            label=f"r{round_index}",
        )
        auxiliary = next(
            (
                values
                for name, values in record["training_history"].items()
                if name != "classifier_loss" and values
            ),
            [],
        )
        if auxiliary:
    ...
    axes[1].set(
        title="Method auxiliary training" if has_auxiliary else "No auxiliary training",
        xlabel="Epoch",
        ylabel="Loss",
    )
    for axis in axes:
    ...
    auxiliary_time = [
        max(
            0.0,
            float(record["timing"].get("method_auxiliary_training_sec", 0.0)),
        )
        for record in records
    ]
    fig, axis = plt.subplots(figsize=(8, 4.5))
