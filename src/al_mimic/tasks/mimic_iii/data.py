...
from .config import require_paths, resolve_path
...
SERIES_AVAILABLE_ARRAYS = ("series_available", "time_series_available", "X_available")
REQUIRED_ARRAYS = (
    "X",
    "s",
    "input_ids",
...
def audit_split_hdf5(config: dict[str, Any]) -> YangWuDataAudit:
    ...
    with h5py.File(paths["split_hdf5"], "r") as handle:
        ...
        for split in SPLIT_NAMES:
            ...
            shapes.update({name: tuple(group[name].shape) for name in OPTIONAL_ARRAYS if name in group})
    ...
    mismatches = [
        f"{name}: expected {required}, got {actual}"
    ...
    with h5py.File(paths["split_hdf5"], "r") as handle:
        root = handle[group_name]
        split_subjects = {
            split: set(np.asarray(root[split][_subject_id_array(root[split])], dtype=np.int64).tolist())
            for split in SPLIT_NAMES
        }
    overlap = {
    ...
    with h5py.File(paths["split_hdf5"], "r") as handle:
        ...
        if "label_names" in root:
            ...
            if len(label_names) != label_count:
                raise ValueError(f"label_names contains {len(label_names)} entries for {label_count} labels")
...
class YangWuFeatureStore:
    ...
    def __init__(self, config: dict[str, Any], *, validate: bool = True) -> None:
        ...
        self.scenario = scenario_from_config(
            ...
            series_available=self._optional_array_any(SERIES_AVAILABLE_ARRAYS, bool),
        )
    ...
    def _load_labels(self) -> np.ndarray:
        ...
        with h5py.File(self.path, "r") as handle:
            root = handle[self.group_name]
            return np.concatenate([np.asarray(root[name]["label"], dtype=np.float32) for name in SPLIT_NAMES])
    ...
    def _load_subject_ids(self) -> np.ndarray:
        ...
        with h5py.File(self.path, "r") as handle:
            ...
            return np.concatenate(
                [
                    np.asarray(root[name][_subject_id_array(root[name])], dtype=np.int64)
    ...
    def _optional_array(self, name: str, dtype: Any) -> np.ndarray | None:
        ...
        with h5py.File(self.path, "r") as handle:
            ...
            if any(name not in root[split] for split in SPLIT_NAMES):
                return None
            return np.concatenate([np.asarray(root[split][name], dtype=dtype) for split in SPLIT_NAMES])

    def _optional_array_any(self, names: tuple[str, ...], dtype: Any) -> np.ndarray | None:
        for name in names:
            value = self._optional_array(name, dtype)
...
class YangWuDataset(Dataset[dict[str, torch.Tensor]]):
    ...
    def __getitem__(self, item: int) -> dict[str, torch.Tensor]:
        ...
        series_missing = self.scenario is not None and self.scenario.series_is_missing(global_index)
        if self.scenario is not None:
            labels = self.scenario.select_labels(labels)
        series = np.asarray(group["X"][local_index], dtype=np.float32)
        result = {
            "time_series": torch.from_numpy(np.zeros_like(series) if series_missing else series),
            ...
            "time_series_available": torch.tensor(not series_missing, dtype=torch.bool),
        ...
        if "time_series_mask" in group:
            series_mask = np.asarray(group["time_series_mask"][local_index], dtype=np.bool_)
            result["time_series_mask"] = torch.from_numpy(
                np.zeros_like(series_mask) if series_missing else series_mask
            )
        if "stay_id" in group:
            result["stay_id"] = torch.tensor(int(np.asarray(group["stay_id"][local_index])), dtype=torch.long)
        return result
...
def prepare_official_artifacts(
    ...
    output_value = output_dir or dataset.get("prepared_dir", "../dataset/prepared/yang_wu_diagnoses_48h")
    output = resolve_path(config, output_value)
