...
def paired_interval(left: Arm, right: Arm, k: int, draws: int, seed: int) -> tuple[float, float, float]:
    ...
    for draw in range(draws):
        ...
        differences[draw] = recall_at_k(right.labels[sample], right.probabilities[sample], k) - recall_at_k(
            left.labels[sample], left.probabilities[sample], k
        )
