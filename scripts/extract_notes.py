...
def join_texts(kept: dict[int, dict[str, tuple[float, int, str]]]) -> pd.DataFrame:
    ...
    rows = [
        (stay, "\n".join(slot[name][2] for name in sorted(slot)))
        for stay, slot in sorted(kept.items())
    ]
...
def tokenize_to_hdf5(
    ...
    source = str(vocab_dir) if (vocab_dir / "vocab.txt").is_file() else "bert-base-uncased"
    tokenizer = BertTokenizer.from_pretrained(source)
    tokenizer.model_max_length = max_tokens
    print(f"tokenizer: {source} (vocab {tokenizer.vocab_size})", flush=True)
...
def main() -> None:
    ...
    parser.add_argument("--vocab-dir", type=Path, default=Path("pretrained/clinicalbert"))
    ...
    stays = icus[icus["ICUSTAY_ID"].isin(set(cohort))][
        ["SUBJECT_ID", "HADM_ID", "ICUSTAY_ID", "INTIME"]
    ]
    print(f"resolved INTIME for {len(stays):,} stays", flush=True)

    kept = collect_latest_notes(
        args.mimic_dir, stays, args.duration, args.chunksize, args.max_chars
    )
