...
def _validate_yang_wu_protocol(config: dict[str, Any]) -> None:
    ...
    profile_name = str(preprocessing.get("protocol_profile", DEFAULT_PROTOCOL_PROFILE))
    profile = _load_protocol_profile(profile_name)
    ...
    # Cohort dimensions come from the named profile: the lock still catches drift,
    # but a second observation window is a reviewed profile entry, not a loosened check.
    integer_values = {
    ...
    integer_values |= {
        # The head follows the scenario's label subset; this pins the artifact itself.
        "model.output_size": (model.get("output_size"), profile["expected_label_count"]),
