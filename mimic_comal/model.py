...
class YangWuGate(nn.Module):
    ...
    def __init__(
        ...
        self.static_weight = (
            nn.Linear(text_dim + time_invariant_dim, 1) if self.has_static else None
        )
...
class YangWuBertEncoderClassifier(nn.Module):
    ...
    The three-modality form preserves the Yang-Wu ICD-9 baseline. The
    two-modality form keeps explicit note and measurement branches for
    ICU-stay phenotyping acquisition; it is a runner-native adaptation rather
    than an exact reproduction of an upstream downstream model.
    ``modality_tokens`` are an exact telescoping decomposition of the fused
    representation and are exposed for active-learning acquisition.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.time_invariant_encoder = (
            nn.Linear(time_invariant_dim, time_invariant_hidden_dim)
            if time_invariant_dim > 0
            else None
        )
    ...
    def encode_modalities(
        ...
        encoded_series = self.time_series_encoder(
            series, src_key_padding_mask=padding_mask
        )
        if self.time_series_pooling == "masked_mean":
            if mask is None:
                mask = torch.ones(
                    encoded_series.shape[:2], dtype=torch.bool, device=encoded_series.device
                )
    ...
    def _decompose(
        ...
        return torch.stack(
            (text_only, fused - text_static, text_static - text_only), dim=1
        )
